...
// swap DMA buffers
    ...
    if (iMode == MODE_DATA && pLCD->pBackBuffer != NULL && !bSetPosition && iFlags & DRAW_TO_RAM) // write it to the back buffer
    {
        ...
        while (i > 0)
        {
            ...
            { Serial.println("going to write past the end of memory, don't even try");
                i = iStrip = 0;
            }
...
GFXglyph *pGlyph;
uint16_t *d;
uint8_t ucBitmap[38]; // 256 pixels wide should be big enough?
...
//            } // for ty
       ...
       // quicker drawing
      } else {// Serial.println("just draw the current character box fast");
         spilcdSetPosition(pLCD, dx, dy, cx, cy, iFlags);
            ...
            for (tx=0; tx<pGlyph->width; tx++) {
               if (bits == 0) { // need to read more font data
                  ...
                  if (k >= sizeof(ucRXBuf)/8/*4*/) { // time to write it
